...
for filename in _template/m-{version,all}/* ; do
  m4 -D__wid=$wid -D__workdir=$workdir -D__srcdir=$src -D__blddir=$bld -D__port=$port -D__bldtype=$bldtype -D__dll=$dll -D__version=$version -D__wwid=$wwid -D__datadir=$workdir/dt $filename > $workdir/$(basename $filename)
done
